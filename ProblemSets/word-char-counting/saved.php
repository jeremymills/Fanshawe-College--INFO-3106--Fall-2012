...
if( isset($_POST['submit']) ) {
	...
	/*if( !empty($form->fields['data']) ) {
		$eqError = rpn_evaluate($form->fields['data']);
		$error = $eqError['has_error'];
		if($error = true)  {
			$form->has_error = true;
...
/*
print rpn_evaluate('1 2 +') . '<br />'; // 3
print rpn_evaluate('3 42 +') . '<br />';// 45
print rpn_evaluate('1024 512 +') . '<br />';// 1536

print rpn_evaluate('1 1 + 2 + 2 +') . '<br />';// 6
print rpn_evaluate('1 1 + 2 2 + -') . '<br />';//4

print rpn_evaluate('10 10 sqrt');
*/
